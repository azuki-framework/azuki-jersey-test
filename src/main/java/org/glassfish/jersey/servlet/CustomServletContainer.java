 ...
 * <p />
 * The servlet or filter may be configured to have an initialization parameter
 * {@value ServletProperties#JAXRS_APPLICATION_CLASS} (see
 * {@link org.glassfish.jersey.servlet.ServletProperties#JAXRS_APPLICATION_CLASS}
 * ) and whose value is a fully qualified name of a class that implements
 * {@link javax.ws.rs.core.Application}. The class is instantiated as a
 * singleton component managed by the runtime, and injection may be performed
 * (the artifacts that may be injected are limited to injectable providers
 * registered when the servlet or filter is configured).
 * <p />
 * If the initialization parameter
 * {@value ServletProperties#JAXRS_APPLICATION_CLASS} is not present and a
 * initialization parameter
 * {@value org.glassfish.jersey.server.ServerProperties#PROVIDER_PACKAGES} is
 * present (see {@link ServerProperties#PROVIDER_PACKAGES}) a new instance of
 * {@link ResourceConfig} with this configuration is created. The initialization
 * parameter
 * {@value org.glassfish.jersey.server.ServerProperties#PROVIDER_PACKAGES} MUST
 * be set to provide one or more package names. Each package name MUST be
 * separated by ';'.
 * <p />
 * If none of the above resource configuration related initialization parameters
 * are present a new instance of {@link ResourceConfig} with
 * {@link WebAppResourcesScanner} is created. The initialization parameter
 * {@value org.glassfish.jersey.server.ServerProperties#PROVIDER_CLASSPATH} is
 * present (see {@link ServerProperties#PROVIDER_CLASSPATH}) MAY be set to
 * provide one or more resource paths. Each path MUST be separated by ';'. If
 * the initialization parameter is not present then the following resource paths
 * are utilized: {@code "/WEB-INF/lib"} and {@code "/WEB-INF/classes"}.
 * <p />
 * All initialization parameters are added as properties of the created
 * {@link ResourceConfig}.
 * <p />
 * A new {@link org.glassfish.jersey.server.ApplicationHandler} instance will be
 * created and configured such that the following classes may be injected onto a
 * root resource, provider and {@link javax.ws.rs.core.Application} classes
 * using {@link javax.ws.rs.core.Context &#64;Context} annotation:
 * {@link HttpServletRequest}, {@link HttpServletResponse},
 * {@link ServletContext}, {@link javax.servlet.ServletConfig} and
 * {@link WebConfig}. If this class is used as a Servlet then the
 * {@link javax.servlet.ServletConfig} class may be injected. If this class is
 * used as a servlet filter then the {@link FilterConfig} class may be injected.
 * {@link WebConfig} may be injected to abstract servlet or filter deployment.
 * <p />
 * Persistence units that may be injected must be configured in web.xml in the
 * normal way plus an additional servlet parameter to enable the Jersey servlet
 * to locate them in JNDI. E.g. with the following persistence unit
 * configuration:
 * 
 * <pre>
 * {@code
 ...
 * </pre>
 * 
 * Given the above, Jersey will inject the
 * {@link javax.persistence.EntityManagerFactory EntityManagerFactory} found at
 * {@code java:comp/env/persistence/widget} in JNDI when encountering a field or
 * parameter annotated with {@code @PersistenceUnit(unitName="WidgetPU")}.
 * 
 * @author Paul Sandoz
 ...
 */
public class CustomServletContainer extends HttpServlet implements Filter, Container {

	private static final long serialVersionUID = 3932047066686065219L;
	private transient FilterConfig filterConfig;
	private transient CustomWebComponent webComponent;
	 ...
	 * @throws javax.servlet.ServletException in case of an initialization
	 *         failure
	 */
	protected void init(WebConfig webConfig) throws ServletException {
		webComponent = new CustomWebComponent(webConfig, resourceConfig);
	 ...
	 */
	public CustomServletContainer(ResourceConfig resourceConfig) {
		this.resourceConfig = resourceConfig;
	}

	/**
	 * Dispatches client requests to the protected {@code service} method.
	 * There's no need to override this method.
	 * 
	 * @param req the {@link HttpServletRequest} object that contains the
	 *        request the client made of the servlet
	 * @param res the {@link HttpServletResponse} object that contains the
	...
	/**
	 * Receives standard HTTP requests from the public {@code service} method
	 * and dispatches them to the {@code do}<i>XXX</i> methods defined in this
	 * class. This method is an HTTP-specific version of the
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/**
		 * There is an annoying edge case where the service method is invoked
		 * for the case when the URI is equal to the deployment URL minus the
		 * '/', for example http://locahost:8080/HelloWorldWebApp
		 */
		 ...
		 * 
		 * On some servlet implementations the getPathInfo() removed contiguous
		 * '/' characters. This is problematic if URIs are embedded, for example
		 * as the last path segment. We need to work around this and not use
		 * getPathInfo for the decodedPath.
		 */
	...
	public void init() throws ServletException {
		init(new CustomWebServletConfig(this));
	}
	 ...
	 *        that contains the request the client made to the Web component.
	 * @param response the {@link javax.servlet.http.HttpServletResponse} object
	 *        that contains the response the Web component returns to the
	 *        client.
	 * @return lazily initialized response status code {@link Value value
	 *         provider}.
	 * @throws IOException if an input or output error occurs while the Web
	 *         component is handling the HTTP request.
	 * @throws ServletException if the HTTP request cannot be handled.
	 */
	public Value<Integer> service(URI baseUri, URI requestUri, final HttpServletRequest request, HttpServletResponse response)
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		if (regex != null && regex.length() > 0) {
			try {
				...
				throw new ContainerException(LocalizationMessages.INIT_PARAM_REGEX_SYNTAX_INVALID(regex,
						ServletProperties.FILTER_STATIC_CONTENT_REGEX), ex);
			}
		}
	...
	/**
	 * Get the servlet context for the servlet or filter, depending on how this
	 * class is registered.
	 * 
	 ...
	 * {@link #service(URI, URI, HttpServletRequest, HttpServletResponse)}
	 * method.
	 * <p />
	 ...
	 * underlying servlet engine can process the request otherwise Jersey will
	 * process the request.
	 * 
	 * @param request the {@link HttpServletRequest} object that contains the
	 ...
	 * @param response the {@link HttpServletResponse} object that contains the
	 *        response the servlet returns to the client.
	 * @param chain the chain of filters from which the next filter can be
	 *        invoked.
	 * @throws java.io.IOException in case of an I/O error.
	 * @throws javax.servlet.ServletException in case of an error while
	...
	public void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
		if (request.getAttribute("javax.servlet.include.request_uri") != null) {
			...
			if (!includeRequestURI.equals(request.getRequestURI())) {
				doFilter(request, response, chain, includeRequestURI, (String) request.getAttribute("javax.servlet.include.servlet_path"),
		 ...
		 * HttpServletRequest#getPathInfo() differently when accessing a static
		 * resource.
		 */
		final String servletPath = request.getServletPath() + (request.getPathInfo() == null ? "" : request.getPathInfo());

		doFilter(request, response, chain, request.getRequestURI(), servletPath, request.getQueryString());
	}

	private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain, String requestURI, String servletPath,
		...
		final URI baseUri = (filterContextPath == null) ? absoluteUriBuilder.replacePath(request.getContextPath()).path("/").build()
				: absoluteUriBuilder.replacePath(request.getContextPath()).path(filterContextPath).path("/").build();

		final URI requestUri = absoluteUriBuilder.replacePath(requestURI).replaceQuery(queryString).build();
	 ...
	 *         {@link ServletProperties#FILTER_STATIC_CONTENT_REGEX}. A
	 *         {@code null} value will be returned if the property is not set or
	 *         is an empty String.
	 */
	...
	public void reload(ResourceConfig configuration) {
		try {
			...
			webComponent = new CustomWebComponent(webComponent.webConfig, configuration);
			containerListener = ConfigHelper.getContainerLifecycleListener(webComponent.appHandler);
